...
def get_dipole_smiles_dict(csv_file):
    ...
    df['dipole_reactant'] = df['rxn_smiles'].apply(lambda x: [smiles for smiles in x.split('>')[0].split('.') if ('+' in smiles and '-' in smiles)][0])
    dipole_smiles_dict = dict(zip(df.index, df.dipole_reactant))

    return dipole_smiles_dict


def correct_reaction_profiles(id, to_run, constrained, preliminary_dir, final_dir):
    ''' 
    Copy initial output folder to final location and decide whether the alternative dipole conformer needs to be included 
    as well (if so, append line to energies.csv and copy .xyz-file)
    '''
    shutil.copytree(os.path.join(preliminary_dir, str(id)), os.path.join(final_dir, str(id)))
    if to_run == True:
        # first get information about the alternative dipole conformer
        try:
            line_alt, energy_alt, name, xyz_alt = get_information_alt_dipole_conf(id)
...
def get_information_alt_dipole_conf(id):
    ''' Read in relevant information from the generated RR conformer of the reactant dipole '''
    with open(os.path.join(os.path.join('lowest_energy_conformers_RR', str(id)), 'energies.csv'), 'r') as f:
            line_alt = f.readlines()[0]
            energy_alt = float(line_alt.split(',')[-1]) + float(line_alt.split(',')[-3])
            name = line_alt.split(',')[0].rstrip('_alt')
            xyz_alt = os.path.join(os.path.join('lowest_energy_conformers_RR', str(id)), f'{line_alt.split(",")[0]}.xyz')
...
if __name__ == '__main__':
    ...
    df_dipoles_alt.apply(lambda x: correct_reaction_profiles(x['rxn_id'], x['to_run'], x['constrained'], args.preliminary_profile_folder, args.finalized_profile_folder), axis=1)
    ...
    print(len(dataset))
    dataset["filter"] = dataset["output"].apply(lambda x: None not in x[0] and None not in x[1])
