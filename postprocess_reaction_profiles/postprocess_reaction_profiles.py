...
def save_rotation_xyzs(home_path, work_dir, id, bond_num):
    ...
    for i in range(6):
        try:
            with open(f"profile_{id}_{bond_num}/{i}.xyz", "w") as f:
                for index in range(
                    energy_indices[i * 10] - 1, energy_indices[i * 10 + 1] - 1
                ):
                    f.write(lines[index])
    ...
    subprocess.run(
        ["tar", "cvzf", f"profile_{id}_{bond_num}.tar.gz", f"profile_{id}_{bond_num}"]
    )
...
def assess_bond_rotatability_from_scan(outfile):
    ...
    with open(outfile, "r") as f:
        ...
        energy_values = [
            float(line.split()[2])
            for line in lines
            if line.startswith("unbiased energy:")
        ]
        bias_energy_values = [
            float(line.split()[2])
            for line in lines
            if line.startswith("    bias energy:")
        ]
    ...
    if (
        abs(energy_values[0] - energy_values[-1]) * hartree > 5
        or max(bias_energy_values) * hartree > 20
        or (max(energy_values) - min(energy_values)) * hartree > 20
        or (max(energy_values) - min(energy_values)) * hartree < 2
    ):
        return False
    else:
        return True
...
def write_scan_input_file(settings_path, dihedral_atoms, init_angle):
    ...
    with open(settings_path, "w") as f:
        ...
        f.write(
            f" dihedral: {dihedral_atoms[0]+1},{dihedral_atoms[1]+1},{dihedral_atoms[2]+1},{dihedral_atoms[3]+1},{init_angle}\n"
        )
        f.write("$end\n")
        f.write("$scan\n")
...
def determine_rotatability(geometry_extractor, home_dir):
    ...
    for i, bond in enumerate(geometry_extractor.dihedral_angles_to_track):
        os.makedirs(
            os.path.join(work_dir, f"scan_{geometry_extractor.rxn_id}_{i}"),
            exist_ok=True,
        )
        os.chdir(os.path.join(work_dir, f"scan_{geometry_extractor.rxn_id}_{i}"))
        input_xyz_scan = os.path.join(
            os.path.join(work_dir, f"scan_{geometry_extractor.rxn_id}_{i}"),
        ...
        settings_path = os.path.join(
            os.path.join(work_dir, f"scan_{geometry_extractor.rxn_id}_{i}"), "scan.inp"
        )
        init_angle = geometry_extractor.ade_ts_mol.dihedral(
        ...
        with open(outfile, "w") as out:
            subprocess.run(
                f"xtb {input_xyz_scan} --alpb water --opt --input {settings_path}",
                shell=True,
                stdout=out,
                stderr=out,
            )
        ...
        shutil.copy(
            outfile, os.path.join(home_dir, f'output_folder/{outfile.split("/")[-1]}')
        )
...
def get_dihedral_angle_atoms(atom1, atom2):
    ...
    try:
        neighbor_atom1 = [
            ...
            if (atom.GetIdx() != atom2.GetIdx() and atom.GetAtomMapNum() != 0)
        ][0]
        neighbor_atom2 = [
            atom
            for atom in atom2.GetNeighbors()
            if (atom.GetIdx() != atom1.GetIdx() and atom.GetAtomMapNum() != 0)
        ][0]
...
def generate_and_select_conformers(geometry_extractor, geom_constraints, home_dir):
    ...
    dipole_mol.name = (
        f'{geometry_extractor.min_xyz_path.split("/")[-1].split(".")[0]}_alt'
    )
    ...
    for dihedral in geom_constraints:
        distance_constraints[(dihedral[0], dihedral[1])] = dipole_mol.distance(
            dihedral[0], dihedral[1]
        )
        distance_constraints[(dihedral[1], dihedral[2])] = dipole_mol.distance(
            dihedral[1], dihedral[2]
        )
        distance_constraints[(dihedral[2], dihedral[3])] = dipole_mol.distance(
            dihedral[2], dihedral[3]
        )
        distance_constraints[(dihedral[0], dihedral[3])] = dipole_mol.distance(
            dihedral[0], dihedral[3]
        )
    ...
    for n in range(1000):
        conformer = conf_gen.get_simanl_conformer(
            dipole_mol, dist_consts=distance_constraints, conf_n=n
        )
    ...
    path = os.path.join(
        os.path.join(home_dir, "lowest_energy_conformers_RR"),
        str(geometry_extractor.rxn_id),
    )
    ...
    return dipole_mol.name, (
        dipole_mol.energy.to("kcalmol")
        - geometry_extractor.ade_r_mol.energy.to("kcalmol")
    )
...
class TSGeomExtractor:
    ...
    def __init__(self, r_smiles, p_smiles, rxn_id, work_dir, geom_path):
        ...
        if not os.path.isdir(geom_path):
            ...
            self._r_mol_unmapped = Chem.AddHs(
                Chem.MolFromSmiles(remove_atom_map_nums(self.r_smiles))
            )
            self._r_mol_map_nums = [
                atom.GetAtomMapNum()
                for atom in Chem.MolFromSmiles(self.r_smiles).GetAtoms()
            ]
            ...
            self._ts_mol_unmapped = Chem.AddHs(
                Chem.MolFromSmiles(remove_atom_map_nums(self.p_smiles))
            )
            ...
            self._reactive_xyz_path = os.path.join(
                self.work_dir, f"reactive_dipole_geom_{self.rxn_id}.xyz"
            )

            # optimize and store dihedral angles to track
            self.write_xyz()
            (
                self.reactive_opt_xyz_path,
                self.ade_r_mol,
                self.ade_ts_mol,
            ) = self.optimize_geom()
    ...
    def get_reactant_xyz_file_name(self, geom_path):
        ...
        file_list = [
            file
            for file in os.listdir(os.path.join(geom_path, str(self.rxn_id)))
            if (file.startswith("r") and file.endswith(".xyz") and "_opt" not in file)
        ]
        for file_name in file_list:
            with open(
                os.path.join(os.path.join(geom_path, str(self.rxn_id)), file_name), "r"
            ) as f:
                lines = f.readlines()
                if (
                    int(lines[0]) == self.r_mol.GetNumAtoms()
                    and len([line for line in lines if not line.startswith("H")]) - 2
                    == self.r_mol.GetNumHeavyAtoms()
                    and len([line for line in lines if line.startswith("C")])
                    == len(
                        [
                            atom
                    ...
                    and len([line for line in lines if line.startswith("N")])
                    == len(
                        [
                            atom
                            for atom in self.r_mol.GetAtoms()
                    ...
                    and len([line for line in lines if line.startswith("O")])
                    == len(
                        [
                            atom
                            for atom in self.r_mol.GetAtoms()
                    ...
                    original_path = os.path.join(
                        os.path.join(geom_path, str(self.rxn_id)), file_name
                    )
    ...
    def extract_lines_from_ts_xyz(self, geom_path):
        ...
        indices_ts_mol_to_retain = [
            self._map_num_dict[map_num] for map_num in self._r_mol_map_nums
        ]
        # Second, translate atom indices from mapped TS to unmapped TS
        line_indices_to_retain = [
            self._ts_mol_ordering[index] + 2 for index in indices_ts_mol_to_retain
        ]
    ...
    def optimize_geom(self):
        ...
        ade_r_mol = ade.Molecule(
            self.min_xyz_path,
            name=self.min_xyz_path.rstrip(".xyz"),
            solvent_name="water",
        )
        ...
        if not len(find_cycles(ade_r_mol.graph)) == len(
            find_cycles(ade_r_smiles_mol.graph)
        ):
            return None, None, None

        # set up ade_ts_mol object
        ade_ts_mol = ade.Molecule(
            self._reactive_xyz_path,
            name=self._reactive_xyz_path.rstrip(".xyz"),
            solvent_name="water",
        )
        ade_ts_mol.optimise(method=ade.methods.XTB())
        ade_ts_mol.reset_graph()

        # if connectivity of ade_r_mol and ade_ts_mol do not match, check if the atoms are the same,
        # and if so, work with the non-optimized geometry
        # else, abort
        if len(find_cycles(ade_r_mol.graph)) == len(find_cycles(ade_ts_mol.graph)):
            ...
            if len(ade_r_mol.atoms) == len(ade_ts_mol.atoms) and [
                atom.label for atom in ade_r_mol.atoms
            ] == [atom.label for atom in ade_ts_mol.atoms]:
    ...
    def get_dihedral_angles_to_track(self):
        ...
        for bond in pos_charged_atom.GetBonds():
            ...
            # If there is a triple bond, or the bond is aromatic (i.e., in a ring),
            # then there is nothing to be constrained -> break off the execution of the function
            if (
                bond.GetBondType() == Chem.rdchem.BondType.TRIPLE
                or bond.GetBondType() == Chem.rdchem.BondType.AROMATIC
            ):
                return dihedral_angles_to_track

        # Finally, add the two bonds, i.e., final_atom -> pos_pole and pos_pole -> min_pole,
        # to the dihedral_angles_to_track list
        for atom1, atom2 in [
            (final_atom, pos_charged_atom),
            (pos_charged_atom, neg_charged_atom),
        ]:
            try:
                dihedral_angles_to_track.append(
                    list(
                        map(
                            lambda x: self._r_mol_ordering[x.GetIdx()],
                            get_dihedral_angle_atoms(atom1, atom2),
                        )
                    )
                )
            except TypeError:
...
def get_compatible_conformer(input_tuple):
    ...
    geometry_extractor = extract_ts_geometry(
        dipole_smiles, product_smiles, rxn_id, home_dir, tmp_dir, geom_dir
    )
    ...
    name, energy_difference = generate_and_select_conformers(
        geometry_extractor, geom_constraints, home_dir
    )
...
def get_all_compatible_conformers(csv_file, geom_dir, num_cores):
    ...
    input_list = get_input_pool(df, pwd, tmp_dir, geom_dir)
        ...
        while True:
            try:
                ...
                if energy_difference != None:
                    ...
                    logger.info(
                        f"Energy difference for {name}, rxn_id {rxn_id} : {energy_difference}"
                    )
                    output_list.append(
                        [name, rxn_id, energy_difference, to_run, constrained]
                    )
                else:
                    logger.info(
                        f"Workflow got interrupted for rxn_id {rxn_id} due to issues with geometries"
                    )
                ...
                logger.info(
                    f"get_compatible_conformers call took more than {error.args} seconds"
                )
                raise
    ...
    with open(
        os.path.join(pwd, "output_postprocessing_dipole_conformers.csv"), "w"
    ) as f:
        f.write(",name,rxn_id,energy_difference,to_run,constrained\n")
        for i, output in enumerate(output_list):
            f.write(
                f"{i}, {output[0]}.xyz,{output[1]},{output[2]},{output[3]},{output[4]}\n"
            )
