...
def get_dipolarophile_map_num_range(reactant_smiles):
    ...
    for smi in reactant_smiles_list:
        if '+' not in smi and '-' not in smi:
            map_num_list = [atom.GetAtomMapNum() for atom in Chem.MolFromSmiles(smi).GetAtoms()] 
        ...
        try:
            ...
            for corr_prod in corr_prods:
                if unmap_unstereo_smiles(corr_prod) not in unmapped_products_no_stereo:
                    rxn_smiles_list.append(f'{map_smiles(reactant_smiles)}>>{corr_prod}')
...
def get_bio_dipolarophiles_list(n=1500):
    ...
    bio_dipolarophiles_list = [
        'C=C', 'C/C=C/C', 'C/C=C\C', #prostaglandines etc.
        'OC(=O)/C=C\C(=O)O', 'OC(=O)/C=C/C(=O)O', #fumaric and maleic acid
        'C1(=O)C=CC(=O)C=C1', 'C1(=O)C(OC)=C(OC)C(=O)C(C)=C1C', #ubiquinone
...
def get_synthetic_dipolarophiles_list(filename="sample_lists/dipolarophiles_sample.csv"):
...
def get_smarts_lists():
    ...
    smarts_list_double, smarts_list_triple, smarts_list_triple2, smarts_list_aromatic = [], [], [], []
    smarts_list_triple.append('([*:1]#[*+:2][*-:3].[C,N:4]=[C:5])>>[*:1]1=[*;+0:2][*;-0:3][C,N:4][C:5]1')
    smarts_list_triple.append('([*:1]#[*+:2][*-:3].[C,N:4]=[C:5])>>[*:1]1=[*;+0:2][*;-0:3][C:5][C,N:4]1')
    smarts_list_triple.append('([*:1]=[*+:2][*-:3].[C:4]#[C:5])>>[*:1]1[*;+0:2][*;-0:3][C:4]=[C:5]1')
    smarts_list_triple.append('([*:1]=[*+:2][*-:3].[C:4]#[C:5])>>[*:1]1[*;+0:2][*;-0:3][C:5]=[C:4]1')

    smarts_list_triple2.append('([*:1]#[*+:2][*-:3].[C:4]#[C:5])>>[*:1]1=[*;+0:2][*;-0:3][C:4]=[C:5]1')
    smarts_list_triple2.append('([*:1]#[*+:2][*-:3].[C:4]#[C:5])>>[*:1]1=[*;+0:2][*;-0:3][C:5]=[C:4]1')

    smarts_list_double.append('([*:1]=[*+:2][*-:3].[C,N:4]=[C:5])>>[*:1]1[*;+0:2][*;-0:3][C,N:4][C:5]1')
    smarts_list_double.append('([*:1]=[*+:2][*-:3].[C,N:4]=[C:5])>>[*:1]1[*;+0:2][*;-0:3][C:5][C,N:4]1')

    smarts_list_aromatic.append('([*:1]:[*+:2][*-:3].[C,N:4]=[C:5])>>[*:1]1[*;+0:2][*;-0:3][C,N:4][C:5]1')
    smarts_list_aromatic.append('([*:1]:[*+:2][*-:3].[C,N:4]=[C:5])>>[*:1]1[*;+0:2][*;-0:3][C:5][C,N:4]1')
    smarts_list_aromatic.append('([*:1]:[*+:2][*-:3].[C:4]#[C:5])>>[*:1]1[*;+0:2][*;-0:3][C:4]=[C:5]1')
    smarts_list_aromatic.append('([*:1]:[*+:2][*-:3].[C:4]#[C:5])>>[*:1]1[*;+0:2][*;-0:3][C:5]=[C:4]1')

    return smarts_list_double, smarts_list_triple, smarts_list_triple2, smarts_list_aromatic
...
def reorder_entries_based_on_num_elec(r_smiles_list):
    ...
    df['num_elec'] = df['reactant_smiles'].apply(lambda x: get_num_elec(x.split('>')[0]))
...
if __name__ == '__main__':
    ...
    synthetic_dipolarophiles_list = get_synthetic_dipolarophiles_list('sample_lists/dipolarophiles_sample.csv')
    smarts_list_double, smarts_list_triple, smarts_list_triple2, smarts_list_aromatic = get_smarts_lists()

    r_smiles_list = get_r_smiles_list(dipole_list, synthetic_dipolarophiles_list)
    ...
    for r_smiles in tqdm(r_smiles_list):
        if ('n' in r_smiles or 'o' in r_smiles):
            rxn_smiles_list = get_rxn_smiles(r_smiles, smarts_list_aromatic, rxn_smiles_list)
        elif r_smiles.count('#') - r_smiles.count('C#N') - r_smiles.count('N#C') == 0:
            rxn_smiles_list = get_rxn_smiles(r_smiles, smarts_list_double, rxn_smiles_list)
        elif r_smiles.count('#') - r_smiles.count('C#N') - r_smiles.count('N#C') == 1:
            rxn_smiles_list = get_rxn_smiles(r_smiles, smarts_list_triple, rxn_smiles_list)
        elif r_smiles.count('#') - r_smiles.count('C#N') - r_smiles.count('N#C') == 2:
            rxn_smiles_list = get_rxn_smiles(r_smiles, smarts_list_triple2, rxn_smiles_list)
    ...
    # initialization for biofragment-based reactions
    rxn_smiles_list2 = []
    dipole_list2 = get_dipole_list("sample_lists/dipoles_sample_bio.csv")
    bio_dipolarophiles_list = get_bio_dipolarophiles_list()

    r_smiles_list2 = get_r_smiles_list(dipole_list2, bio_dipolarophiles_list)
    r_smiles_list2 = reorder_entries_based_on_num_elec(list(set(r_smiles_list2)))

    # get the reaction SMILES for each reactant combination
    for r_smiles in tqdm(r_smiles_list2):
        if ('n' in r_smiles or 'o' in r_smiles):
            rxn_smiles_list2 = get_rxn_smiles(r_smiles, smarts_list_aromatic, rxn_smiles_list2)
        elif r_smiles.count('#') - r_smiles.count('C#N') - r_smiles.count('N#C') == 0:
            rxn_smiles_list2 = get_rxn_smiles(r_smiles, smarts_list_double, rxn_smiles_list2)
        elif r_smiles.count('#') - r_smiles.count('C#N') - r_smiles.count('N#C') == 1:
            rxn_smiles_list2 = get_rxn_smiles(r_smiles, smarts_list_triple, rxn_smiles_list2)
        elif r_smiles.count('#') - r_smiles.count('C#N') - r_smiles.count('N#C') == 2:
            rxn_smiles_list2 = get_rxn_smiles(r_smiles, smarts_list_triple2, rxn_smiles_list2)
    ...
    with open('reactions_biofrag_finalized.csv', 'w') as f:
        for rxn_smiles in rxn_smiles_list2:
            if rxn_smiles not in rxn_smiles_written_bio:
                ...
                rxn_smiles_written_bio.append(rxn_smiles)
                j += 1
            else:
                continue
